...
class MegaStarmiePolicy(BasePolicy):
    ...
    def score_attach(self, o):
        ...
        if src is not None and src.id == C.IGNITION_ENERGY:
            ...
            if opp is not None and (self.effect_prevented(opp) or (120 < opp.hp <= 210)):
                return 9300                    # Nebula KO / pierce that Jetting Blow can't land
    ...
    def score_play_trainer(self, card):
        ...
        if cid == C.MEGA_SIGNAL:
            # PrizeTracker: if every Mega Starmie ex is prized/used, this search whiffs -> skip it.
        ...
        if cid == C.CRUSHING_HAMMER:
            return 4000 if (opp is not None and len(opp.energies) >= 1) else 200
