...
class AlakazamPolicy:
    ...
    def _opp_has_counter_attacker(self):
        """Opponent fields a counter-placement / effect-damage attacker (Powerful Hand
        Alakazam line) -> our attacker wants Mist Energy, which negates 'place counters'."""
        for p in (self.opponent.active + self.opponent.bench):
            if p is None:
                continue
            if p.id in ALAKAZAM_IDS or p.id in (C.ABRA, C.KADABRA):
                return True
            c = card_table.get(p.id)
            if c and POWERFUL_HAND in (c.attacks or []):
                return True
        return False
    ...
    def _score_attach(self, o):
        ...
        # DEFENSIVE MIST: vs a counter-placement opponent (Alakazam's Powerful Hand), overlay a
        # Mist/effect-prevent energy onto our ATTACKER so its Powerful Hand does 0 to us. Mist
        # provides {C} (cannot pay the {P} cost), so only do it once the body ALREADY has its
        # {P} attack online — never trade away offense for it. Priority: the active attacker
        # first, then a benched Alakazam already fuelled (the next attacker being prepared).
        # Prize gate: Mist is a survival/comeback tool. When AHEAD on prizes we'd rather spend the
        # attach racing/developing, so never Mist while ahead; tie or behind may overlay it.
        # (Offense always wins the tie-break — fuelling an attacker scores 8000+ > Mist's 7900.)
        not_ahead = len(self.me.prize) >= len(self.opponent.prize)
        if (src is not None and src.id in EFFECT_PREVENT_ENERGY
                and p.id in ALAKAZAM_IDS
                and self._can_attack(p)              # already has its {P}; Mist is pure overlay
                and not self._effect_prevented(p)    # not already protected
                and not_ahead
                and self._opp_has_counter_attacker()):
            return 7900 if o.inPlayArea == AreaType.ACTIVE else 7600
