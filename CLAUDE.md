...
## Current status (2026-06-22 AM)
- **DECK SWITCH → DRAGAPULT EX IS NOW OUR PRIMARY.** The meta flipped AGAIN (6-21 episodes): Dragapult ex **surged into the top tier (16.5% / 63.1% WR)** and **hard-counters our Trevenant (79%)**. Trevenant's top-tier WR collapsed **64.6%→52.3%** as its counters (Dragapult, Chandelure, Cinderace, Froslass) all rose at once and its prey (Lucario) went extinct (0.4%). Our score stalled 911→**879.8** (6-21 A/B: baseline 879.8 ≈ v2 877.8 = the TO_HAND/DISCARD piloting v2 was FLAT/noise, no help).
- **6-22 SHIPPED (2/5 subs, 3 left): Dragapult ex (NEW primary) + Alakazam (hedge).** Pair = our latest-2 scored; displayed = best of the two. Scores accrue after next reset (UTC 00:00 / TW 08:00) — **CHECK THEM**.
- **`agents/dragapult/` (NEW) — Dragapult ex.** Consensus top-tier netdeck (ONE list shared by all 5 Elo≥1150 pilots, 504 games / 63% WR). Stage-2 spread/control: Dreepy→Drakloak→Dragapult ex (Rare Candy skip), **Phantom Dive [R][P]=200 +6 bench damage-counters** for multi-KO turns, **4× Crushing Hammer** energy disruption, Budew item-lock, Boss snipe. **PILOT = the official tuned sample agent** (`docs/official/models/a-sample-rule-based-agent-dragapult-ex-deck.ipynb`) wrapped in our robust scaffolding (cwd-safe deck-load + try/except fallback + diag). Why adopt it: our hand-rolled `DragapultPolicy` lost to the sample **13-1** in cabt (it under-pilots energy/evolve/Phantom-Dive-spread); the sample plays the EXACT consensus deck, goes SECOND, plans the 6-counter spread to maximize prizes, and detects KOs via `obs.logs`. After adoption: cabt **50% vs the sample (= faithful), 60% vs Lucario, 83% vs our Trevenant** (matches the real-meta 79%).
  - **LESSON (new): when an official sample bot exists for the deck you're piloting, START from it.** It's a competent, deck-specific baseline that beat a from-scratch policy 13-1. Only the robustness wrapper is our value-add. (Samples live in `docs/official/models/*.ipynb`: crustle, lucario, abomasnow, **dragapult**, iono.)
- `agents/alakazam/` — 胡地小人 (AlakazamPolicy, **2nd deck / hedge**). Even vs Dragapult in the real meta (49%), 55.1% top-tier WR (now HIGHER than Trevenant's 52.3%), beats Lucario. But OUR pilot under-performs the archetype (6-21 ladder 674.6; cabt-loses 80% to our Dragapult) — piloting headroom remains.
- `agents/trevenant/` — Hop's Trevenant (TrevenantPolicy). **DEMOTED 6-22**: rode the Lucario wave, but Lucario died and Trevenant's counters surged → top-tier WR 52.3% and falling; ladder 879.8. Keep as a pivot if the field swings back. Working tree still has the FLAT v2 piloting diff (877.8 ≈ 879.8 baseline) — uncommitted, harmless.
- `agents/bellibolt/` (836, crashed in meta) + `agents/typhlosion/` (532) — DEPRIORITIZED. `agents/alakazam_mist/` — regressed (907.8 < 1006.7), do NOT use.
- **CRITICAL LESSON: local sims (ctypes + cabt) do NOT reliably predict ladder rank** — many local "optimizations" LOWERED the score (Mist cabt 62% but ladder 907<1006). BUT the Dragapult call is the best-supported deck decision yet: real-meta #1 WR + faithfully-adopted tuned pilot + cabt cross-checks ALL agree (vs past cabt-ONLY misfires). Still: REAL-LADDER A/B is the only judge — confirm at reset.
- **divergence_decode GOTCHAS**: pass the EXACT meta archetype name (e.g. `--archetype "Hop's Trevenant"`); contexts print sorted by agree% ASC so `| tail -N` silently drops low-agree ones — pipe to a file/`grep`, never tail. Don't run many decode jobs in parallel (CPU pileup → hang).
- Daily limit resets **UTC 00:00 = Taiwan 08:00**. 5 subs/day; latest 2 scored (displayed = best of the 2). Today (6-22) used **2/5** = {Alakazam hedge, Dragapult primary}. 3 subs left.
...
## Next steps / ideas
1. **Check the 6-22 scores next reset (THE key read)**: did **Dragapult ex** beat our 879.8 floor? (displayed = best of {Dragapult, Alakazam}.) If Dragapult ≥ ~900, the deck switch worked — keep it primary and mine its piloting. If it disappoints (<879), we still have 3 subs/day to resubmit Trevenant-879 as a floor while diagnosing. This is the one place cabt could still be lying.
2. **Mirror + Trevenant are most of Dragapult's field** (Trevenant 37.6% + Alakazam 27.6% + Dragapult mirror rising). Edges come from piloting. The sample pilot is decent but not top-tier — **mine Dragapult divergences vs the Elo≥1150 pool** (`tools/divergence_decode.py --archetype "Dragapult ex"`; pipe to file, never `| tail`). Likely gaps: Phantom-Dive spread placement, Crushing-Hammer targeting, Boss timing, when to go for the multi-KO turn.
3. **Watch CINDERACE (Dragapult's only clear loss, 36%) + Chandelure + Mega Froslass ex** — as Dragapult/Trevenant define the field, their counters rise next. Pull Cinderace's decklist/mechanics from episodes; find a tech or sideboard answer if it climbs.
4. **Alakazam piloting headroom**: archetype is 55% top-tier WR but our pilot scored only 674 and cabt-loses 80% to our Dragapult. No official Alakazam sample exists, so improve via divergence mining vs top Alakazam pilots (Elo≥1150). It's our hedge if the field turns anti-Dragapult.
5. **When piloting any deck that has an official sample** (`docs/official/models/*.ipynb`: crustle/lucario/abomasnow/dragapult/iono), START from the sample (it beat a from-scratch policy 13-1) and only add the robustness wrapper.
6. Re-pull daily episode data; `tools/meta_analyze.py --elo 1150` every day — the meta flips fast (Lucario 56%→extinct, Trevenant 10%→43%→WR-crash, Dragapult tiny→16.5%/63% top tier, all within days).
