...
- **6-21 PILOTING fixes shipped (mimic top pilots via NEW `tools/divergence_decode.py`)**: it decodes each agree/disagree into card/attack names (`--player` isolates ONE pilot). Rank-1 = **"The Debauchery Tea Party" (Trevenant, 1336)** plays ≈our deck → pure-piloting A/B. Trevenant: TO_ACTIVE 47→67% (promote Cramorant>Phantump>Trevenant, NOT Fighting-weak Snorlax), TO_HAND 59→74% (favor draw engine over Boss/Band), kept GO-SECOND. Alakazam (Elo≥1150 pool): IS_FIRST 5→95% (GO FIRST), TO_ACTIVE 78→94% (promote Abra/Kadabra line, not the Dudunsparce wall). **LESSON: IS_FIRST is DECK/PILOT-specific** — Alakazam pool goes first 35/35, but Debauchery's Trevenant goes second 15/20 (aggro attacks first turn); mimic the #1 with `--player`, not the noisy pool. Reverted an Alakazam "draw-less" hand-cap (≈0 divergence gain, regressed cabt 53→35). Both improved agents SUBMITTED 6-21 (same-field A/B). See memory [Top Pilot Targets].
...
- Daily limit resets **UTC 00:00 = Taiwan 08:00**. 5 subs/day; latest 2 scored (displayed = best of the 2). Today (6-21) used 2/5 = {improved Trevenant, improved Alakazam}.
...
## Next steps / ideas
1. **Check the 6-21 A/B next reset**: improved Trevenant vs improved Alakazam (both submitted). agree% is only a PROXY — the ladder score decides. To prove a fix worked, do a SAME-DECK before/after A/B (improved vs current) on one field, not improved-vs-improved.
2. **Keep mining piloting fixes with `divergence_decode.py`** (then ladder-A/B each): remaining Trevenant gaps = MAIN (39%, mostly action-ORDERING = inherently low, low value), DISCARD. Alakazam = TO_DECK (1% agree — unclear mechanic, investigate the card first), and the "draw LESS like top pilots" idea (top pool activates Run Away Draw ~1/4 as often) — needs a surgical rule (only when already lethal), NOT a blunt hand-cap (that regressed cabt 53→35).
3. **Always `--player` the rank-1 pilot when one dominant pilot runs our deck** (Debauchery for Trevenant). The Elo≥1150 POOL mixes other builds (e.g. Dudunsparce Trevenant) and gave WRONG signals (it said go-first; Debauchery goes second). Pool is fine for Alakazam (multiple consistent pilots).
4. Try the 94-count Trevenant variant (Trevenant+Dudunsparce engine, 77% wild WR vs the 68% Team-Rocket build we shipped).
5. Re-pull daily episode data; `tools/meta_analyze.py` every day — the meta flips fast (Crustle ~50%→dead in 2 days).
